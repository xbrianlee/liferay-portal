...
public class FieldTypeaheadDataProviderSearchHelper {
	...
	private void _addTermFilterClauses(
		...
		BooleanQuery termFilterClauses = _getTermFilters(dataProviderSettings);
	...
	private Query _getBoolPrefixMatchQuery(
		...
		String fuzziness = GetterUtil.getString(
			dataProviderSettings.getAttribute("fuzziness"));

		if (!Validator.isBlank(fuzziness)) {
			multiMatchQuery.setFuzziness(GetterUtil.getString(fuzziness));
		}

		Integer prefixLength = GetterUtil.getInteger(
	...
	private BooleanQuery _getTermFilters(
		DataProviderSettings dataProviderSettings) {
	...
	private void _setIndices(
		...
		String[] indices = GetterUtil.getStringValues(
			dataProviderSettings.getAttribute("indices"));
	...
	private void _setSorts(
		...
		if ((dataProviderSettings == null) || (sortFieldMap == null)) {
			return;
		}
