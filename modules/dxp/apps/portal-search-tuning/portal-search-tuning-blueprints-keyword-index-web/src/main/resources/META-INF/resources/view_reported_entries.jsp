...
<%
ViewReportedEntriesManagementToolbarDisplayContext viewReportedEntriesManagementToolbarDisplayContext = (ViewReportedEntriesManagementToolbarDisplayContext)request.getAttribute(KeywordIndexWebKeys.REPORTED_ENTRIES_MANAGEMENT_TOOLBAR_DISPLAY_CONTEXT);
	...
	<aui:form method="post" name="fm">
		...
		>
			<liferay-ui:search-container-row
				className="com.liferay.portal.search.tuning.blueprints.keyword.index.index.KeywordEntry"
				keyProperty="keywordEntryId"
				modelVar="entry"
			>
				<%@ include file="/reported_entry_search_columns.jspf" %>
			</liferay-ui:search-container-row>
...
<liferay-frontend:component
	componentId="<%= viewReportedEntriesManagementToolbarDisplayContext.getDefaultEventHandler() %>"
	module="js/view_reported_entries/ViewReportedEntriesManagementToolbarDefaultEventHandler"
/>

<aui:script sandbox="<%= true %>">
	var submitForm = function (url) {
		var searchContainer = document.getElementById(
			'<portlet:namespace />reportedEntries'
		);
	...
	Liferay.componentReady('reportedEntriesManagementToolbar').then(
		(managementToolbar) => {
